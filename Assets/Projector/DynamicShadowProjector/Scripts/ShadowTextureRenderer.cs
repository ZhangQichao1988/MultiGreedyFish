...
#pragma warning restore // CS1692, RECS0001
        ...
        private TextureMultiSample m_multiSampling = TextureMultiSample.x1;
        [SerializeField]
		...
		void CreateTemporaryRenderTarget()
		{
			if (useIntermediateTexture)
			{
				...
				m_temporaryRenderTarget = RenderTexture.GetTemporary(width, height, 0, m_shadowTexture.format, RenderTextureReadWrite.Linear, (int)m_multiSampling);
		...
		{
			RenderTargetIdentifier finalDstId = new RenderTargetIdentifier(m_shadowTexture);
			int tempRTIndex = 0;
			if (!useIntermediateTexture)
			{
				srcId = finalDstId;
			...
			else
			{
				if (m_superSampling != TextureSuperSample.x1 || HasShadowColor())
				{
					...
					if (0 < m_blurLevel)
					{
						cmd.GetTemporaryRT(s_temporaryRenderTargetNameIds[tempRTIndex], m_textureWidth, m_textureHeight, 0, FilterMode.Bilinear, m_shadowTexture.format, RenderTextureReadWrite.Linear);
			...
			if (0 < m_mipLevel)
			{
				...
				cmd.GetTemporaryRT(s_temporaryRenderTargetNameIds[tempRTIndex], w, h, 0, FilterMode.Bilinear, m_shadowTexture.format, RenderTextureReadWrite.Linear);
