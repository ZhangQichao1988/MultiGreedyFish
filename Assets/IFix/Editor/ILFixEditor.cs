        ...
        public static void Patch()
        {
            ...
            try
            {
                foreach (var assembly in injectAssemblys)
                {
                    ...
                    FinalMove("editor", string.Format("{0}.patch.bytes", assembly));
                }
            }
            catch (Exception e)
            {
...
#endif
        public static string targetPath = Application.dataPath + "/../xlsx/patch";
        static void FinalMove(string platform, string pathName = "Assembly-CSharp.patch.bytes")
        {
            string srcPath = Application.dataPath + "/../" + pathName;
            string finalTargetPath = Path.Combine(targetPath, platform);
            string destPath = Path.Combine(finalTargetPath, pathName);
